...
(defvar *pj/org-default-refile-file*
  (concat (file-name-as-directory *pj/org-directory*) "refile.org"))
(defvar *pj/org-default-idea-file*
  (concat (file-name-as-directory *pj/org-directory*) "ideas.org"))
...
:END:" :prepend t :clock-in t :clock-resume t)
          ...
          ("i" "idea" entry (file *pj/org-default-refile-file*)
           "* %? :idea:
:PROPERTIES:
:ID %(pj/uuidgen)
:CREATED: %U
...
;; Work in progress
(defun pj/activate-idea ()
  (interactive)
  "Take idea entry under cursor, and make active task"
  (let* ((subtree-end (save-excursion (org-end-of-subtree t)))
         (properties (org-agenda-with-point-at-orig-entry nil ())))
    
     ;; Check if current element is an idea - tagged :idea: without todo state
     (prin1 properties)
     ;; Remove idea tag
     ;; Add todo state tag
     ;; Move to refile (possibly offer refile location interactively)
     ))
